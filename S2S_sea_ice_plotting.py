...
# RMSE ratio plots
### Plot RMSE as a function of lead time for each model
def RMSE_plot_ratio(ax_plt,data_plt,data_scatt,reg_sel,TO_PLOT,iplt,v_min,v_max,no_regions,ncols,nrows):
    # subplot panel labels
    panel_lett = ['a','b','c','d','e','f',
                 'g','h','i','j','k','l']
    # Palette
    ...
    sns.lineplot(data=data_plt,x='lead days',y=TO_PLOT,hue='Model Name',
                 ax=ax_plt,markers=False,linewidth=6,legend=False if iplt < no_regions - 1 else 'full')
    # Scatter plot on top
    sns.scatterplot(data=data_scatt,x='lead days',y=TO_PLOT,hue='Model Name',
                             ax=ax_plt,legend=False,s=350)
    # 
    ax_plt.grid()
    ax_plt.set_ylim((v_min,v_max))
    ...
    sns.despine(ax=ax_plt)
   ### Plot RMSE as a function of lead time for each model
def RMSE_plot_actual(ax_plt,data_plt,data_scatt,reg_sel,TO_PLOT,iplt,v_min,v_max,no_regions,ncols,nrows):
    # subplot panel labels
    panel_lett = ['a','b','c','d','e','f',
                 'g','h','i','j','k','l']
    # Palette
