...
no_ens = 25 ##no. of ensemble members
...
itime = 0
init_select = init_times[itime].to_dataset()
print(init_select)
##We'll create another DataFrame inside this loop; we'll append it 
##to the big DataFrame outside of this loop.
d_SIC_lead_time = pd.DataFrame({"I (init date)":init_select.repeat(len(forecast_times)*no_ens),
                            "V (valid date)":"",
...
date_change = np.repeat(init_select.values,len(forecast_dates)) + forecast_dates
d_SIC_lead_time['V (valid date)'].iloc[save_ind] = pd.to_datetime(date_change).date
